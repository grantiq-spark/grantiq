 ...
 * 구조: 긍정팀/부정팀 토론 → 4명 임원 의견 → 오케스트레이터 최종 결정
 */
...
async function runDebate(opportunity, memory) {
  ...
  const [proRes, conRes] = await Promise.all([
    callClaudeDebate("당신은 적극적인 사업 추진론자입니다. 아래 공고를 우리 회사가 반드시 해야 하는 3가지 이유를 설명하세요. 근거와 기회면을 강조하세요.\n\n회사: " + companyCtx + "\n공고: " + grantCtx),
    callClaudeDebate("당신은 신중한 리스크 관리자입니다. 아래 공고를 우리 회사가 신중해야 하는 리스크를 설명하세요. 현실적 우려사항을 구체적으로 제시하세요.\n\n회사: " + companyCtx + "\n공고: " + grantCtx),
  ]);
...
export async function runBoardReview({ opportunity, slackChannel, threadTs }) {
  ...
  try {
    ...
    const { pro, con } = await runDebate(opportunity, memory);

    await postMessage({
      channel: slackChannel,
      text: "⚖️ 이사회 사업 토론",
      blocks: debateBlocks(pro, con),
      thread_ts: threadTs
    });
  } catch (e) {
    console.error("[boardReview] debate failed:", e);
  }
