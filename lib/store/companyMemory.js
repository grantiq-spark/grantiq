...
export async function upsertCompanyProfile(profile) {
  ...
  if (existing) {
    return dbUpdate("companies", DEFAULT_COMPANY_ID, { ...profile, updated_at: new Date().toISOString() });
  }
  return dbInsert("companies", { id: DEFAULT_COMPANY_ID, ...profile });
}

export async function addDocument(doc) {
  return dbInsert("source_documents", { company_id: DEFAULT_COMPANY_ID, ...doc });
}
...
export async function buildMemoryContext() {
  ...
  return {
    company_name: company?.name || process.env.NEXT_PUBLIC_COMPANY_NAME || "GRANTIQ",
    description: company?.description || "R&D 겵고경스 사업 큙숸 SAaS",
