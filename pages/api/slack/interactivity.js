...
export default async function handler(req, res) {
  ...
  Promise.resolve().then(async () => {
    const { action_id: aid, value } = action;
    if (aid === "start_board_review") {
      const opp = await dbGet("opportunities", value).catch(() => null);
      if (!opp) { await postMessage({ channel, thread_ts: threadTs, text: `❌ 공고을: ${value}` }); return; }
      await postMessage({ channel, thread_ts: threadTs, text: `🏛️ 이사회 심의 시작</code> ${opp.title}` });
      await runBoardReview({ opportunity: opp, slackChannel: channel, threadTs });
    } else if (aid === "generate_proposal") {
      await generateProposal({ opportunityId: value, slackChannel: channel, threadTs });
    } else if (aid === "refresh_memory") {
      const mem = await buildMemoryContext();
      await postMessage({ channel, thread_ts: threadTs, text: `🧠 메모리 검: ${mem.company_name}` });
    } else if (aid === "rerun_monitor") {
      await postMessage({ channel, thread_ts: threadTs, text: "🔍 재실행..." });
      ...
      for (const opp of opps.slice(0,3)) {
        ...
        if (v.fit_score >= 55) await postMessage({ channel, thread_ts:threadTs, text:`📋 공고: ${opp.title} (${v.fit_score}점)`, blocks:boardPacketBlocks(rec) });
      }
      await postMessage({ channel, thread_ts:threadTs, text: `✅ 완료 — ${opps.length}개 검색` });
    }
  }).catch(err => {
