...
export default async function handler(req, res) {
  ...
  let payload;
  try { payload = JSON.parse(rawStr); } catch { return res.status(400).json({ error: "Invalid JSON" }); }
  ...
  }).catch(err => console.error("[events]", err));
}
...
async function handleFileShared(event) {
  ...
  await postMessage({ channel: channel_id, text: "Analyzing file...", blocks: ingestionProgressBlocks("file", "processing", "extracting memory") });
  try {
    const r = await ingestSlackFile(file_id);
    const caps = (r.extracted && r.extracted.capabilities && r.extracted.capabilities.length) || 0;
    const projs = (r.extracted && r.extracted.past_projects && r.extracted.past_projects.length) || 0;
    const snips = (r.extracted && r.extracted.evidence_snippets && r.extracted.evidence_snippets.length) || 0;
    await postMessage({ channel: channel_id, text: "Done: " + r.name, blocks: ingestionProgressBlocks(r.name, "done", r.text_length + "chars caps:" + caps + " projs:" + projs + " evidence:" + snips) });
...
async function handleAppMention(event) {
  ...
  if (!cmd || lower.includes("help")) {
    await postMessage({ channel, thread_ts: ts, text: "*GRANTIQ*\n- Upload file => memory update\n- monitor now => search\n- memory status => show memory\n- review <id> => board review\n- proposal <id> => draft\n- status => system info" });
    return;
  }
  ...
  if (lower.includes("monitor") || lower.includes("search")) {
    ...
    try {
      ...
      for (const opp of opps.slice(0, 5)) {
        ...
        if (v.fit_score >= 55) { await postMessage({ channel, thread_ts: ts, text: "Found (" + v.fit_score + "): " + opp.title, blocks: boardPacketBlocks(rec) }); posted++; }
      }
  ...
  if (lower.includes("status")) {
    const [o, rv, d] = await Promise.all([dbList("opportunities").catch(() => []), dbList("board_reviews").catch(() => []), dbList("proposal_drafts").catch(() => [])]);
    await postMessage({ channel, thread_ts: ts, text: "Status: opps:" + o.length + " reviews:" + rv.length + " drafts:" + d.length + (!process.env.DATABASE_URL ? "\n[WARNING] in-memory DB" : "") });
    return;
  }
